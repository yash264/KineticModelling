...
for beta, rec in data.items():
    ...
    fT = interp1d(alpha[::step], T[::step], fill_value="extrapolate", assume_sorted=True)
...
csv_path = os.path.join(RESULTS_DIR, "daem_results.csv")
df_res.to_csv(csv_path, index=False)

plt.xlabel("1/T (1/K)", fontsize=12)
plt.ylabel("ln(β / T²)", fontsize=12)
plt.title("DAEM Analysis: ln(β/T²) vs 1/T", fontsize=14)
